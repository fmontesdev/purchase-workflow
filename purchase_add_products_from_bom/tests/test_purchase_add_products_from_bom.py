...
class PurchaseAddProductsFromBoMTest(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.purchase_order = cls.env["purchase.order"]
        cls.add_product_from_bom_wizard = cls.env["add.products.from.bom"]
    ...
    def test_purchase_add_products_from_bom(self):
        ...
        wizard.add_products()
        # Validate against the actual selected products after explode/return.
        expected_first_products = set(
            wizard.raw_product_line_ids.filtered("selected_product")
            .mapped("product_id")
            .ids
        )
        self.assertEqual(len(purchase.order_line), len(expected_first_products))
        ...
        expected_second_products = set(
            wizard_2.raw_product_line_ids.filtered("selected_product")
            .mapped("product_id")
            .ids
        )
        expected_total_products = expected_first_products | expected_second_products
        self.assertEqual(len(purchase.order_line), len(expected_total_products))

    def test_return_finished_products_from_bom_second_level(self):
        """Test return finished products from BoM at second level."""
        ...
        root_template = template_model.create(
            {
                "name": "Root Nested BoM",
                "purchase_ok": True,
                "sale_ok": False,
                "type": "consu",
                "uom_id": unit_uom.id,
        ...
        level_1_template = template_model.create(
            {
                "name": "Level 1 Nested BoM",
                "purchase_ok": True,
                "sale_ok": False,
                "type": "consu",
                "uom_id": unit_uom.id,
        ...
        level_2_template = template_model.create(
            {
                "name": "Level 2 Nested BoM",
                "purchase_ok": True,
                "sale_ok": False,
                "type": "consu",
                "uom_id": unit_uom.id,
        ...
        leaf_template = template_model.create(
            {
                "name": "Leaf Nested BoM",
                "purchase_ok": True,
                "sale_ok": False,
                "type": "consu",
                "uom_id": unit_uom.id,
        ...
        root_product = root_template.product_variant_id
        level_1_product = level_1_template.product_variant_id
        level_2_product = level_2_template.product_variant_id
        leaf_product = leaf_template.product_variant_id

        root_bom = bom_model.create(
            {
                "product_tmpl_id": root_template.id,
        ...
        bom_model.create(
            {
                "product_tmpl_id": level_1_template.id,
                "product_uom_id": unit_uom.id,
                "product_qty": 1,
                "bom_line_ids": [
                    (
        ...
        bom_model.create(
            {
                "product_tmpl_id": level_2_template.id,
                "product_uom_id": unit_uom.id,
                "product_qty": 1,
                "bom_line_ids": [
                    (
        ...
        wizard_values = wizard_obj.default_get(wizard_obj.fields_get().keys())
        wizard_values.update(
            {
        ...
        level_1_line = wizard.raw_product_line_ids.filtered(
            lambda line: line.product_id == level_1_product
        )
        level_1_line.add_products_from_bom()

        level_2_line = wizard.raw_product_line_ids.filtered(
            lambda line: line.product_id == level_2_product
        )
        level_2_line.add_products_from_bom()

        leaf_line = wizard.raw_product_line_ids.filtered(
            lambda line: line.product_id == leaf_product
        )
        self.assertTrue(leaf_line)
        self.assertEqual(leaf_line.parent_line_id, level_2_line)

        level_2_line.return_finished_products_from_bom()

        self.assertFalse(
            wizard.raw_product_line_ids.filtered(
                lambda line: line.product_id == leaf_product
            )
        )
        self.assertTrue(
