...
class AddProductsFromBoM(models.TransientModel):
    ...
    def button_get_products_from_bom(self):
        ...
        view_form = self.env.ref(
            "purchase_add_products_from_bom.add_products_from_bom_wizard_view"
        )
        return {
    ...
    def get_products_from_bom(self, finished_product, product_qty, bom_id):
        product_uom_id = finished_product.uom_po_id or finished_product.uom_id
        material_list = []

        factor = (
            product_uom_id._compute_quantity(product_qty, bom_id.product_uom_id)
            / bom_id.product_qty
        )
    ...
    def _simulate_purchase_line_onchanges(self, product, product_qty, product_uom):
        ...
        values = purchase_line_obj.default_get(purchase_line_obj.fields_get().keys())
        ...
        values.update(purchase_line._prepare_add_missing_fields(values))
    ...
    def add_products(self):
        for line in self.raw_product_line_ids:
            ...
            if product_already_in_line:
                ...
                values = self._get_purchase_line_values(line)
                self.purchase_id.order_line.create(values)

    def _get_purchase_line_values(self, line):
        purchase_line_obj = self.env["purchase.order.line"]
        values = purchase_line_obj.default_get(purchase_line_obj.fields_get().keys())
        values.update(
            {
...
class AddProductsFromBoMLines(models.TransientModel):
    ...
    product_finished_id = fields.Many2one(comodel_name="add.products.from.bom")
    ...
    product_qty = fields.Float(
        string="Quantity to Buy", digits="Product Unit of Measure", required=True
    )
    ...
    product_qty_stock = fields.Float(
        string="Quantity Available in Stock", related="product_id.qty_available"
    )
    ...
    # Keep the direct parent-child relationship between exploded lines.
    parent_line_id = fields.Many2one(
        comodel_name="add.products.from.bom.lines",
        string="Parent Exploded Line",
        readonly=True,
    )
        ...
        for record in self:
            ...
            if record.purchase_uom_id != record.product_uom_id:
                product_qty = record.purchase_uom_id._compute_quantity(
                    record.product_qty, record.product_uom_id
                )

            record.product_qty_after_buy = record.product_qty_stock + product_qty
    ...
    def _compute_total_product_value(self):
        for record in self:
            record.total_product_value = record.price_unit * record.product_qty
    ...
    def add_products_from_bom(self):
        ...
        material_list = self.product_finished_id.get_products_from_bom(
            self.product_id, self.product_qty, self.bom_id
        )
    ...
    def _get_descendant_lines(self):
        self.ensure_one()
        child_lines = self.product_finished_id.raw_product_line_ids.filtered(
            lambda line: line.parent_line_id == self
        )
        descendant_lines = child_lines
        for child_line in child_lines.filtered("exploded_product"):
            descendant_lines |= child_line._get_descendant_lines()
        return descendant_lines

    def return_finished_products_from_bom(self):
        products_to_delete = self._get_descendant_lines()
